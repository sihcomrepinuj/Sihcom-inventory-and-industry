...
app.py              Flask web interface (deployable to Railway)
    ...
    └── build_list.py  Build-list persistence (build_list.json: {targets, buy_set}; build_station is per target)
...
- `Target` dataclass — one build-list target plus its resolved input tree (`needed` = product units = runs * qty_per_run). Carries a per-target `build_station` (`int|None`; `None` = "owned anywhere"), so each product is planned against its own station.
...
- `attach_owned_totals(rows, owned_index, volumes)` — for the `/materials` flat view: nets each row's gross requirement against a **flat owned-anywhere** index (no station concept), adding `total` / `owned` / `to_buy` (+ volume). Pure; returns new dicts. Backs the aggregated bill of materials.
- `attach_haul_breakdown(rows, loc_names)` — pure helper that adds a display-ready `haul` list (`[{name, qty}]`, sorted by qty desc) to each row from its `elsewhere` {station_id: qty} map, resolving ids via `loc_names` (falling back to `str(id)`). Returns new dicts; never mutates inputs. Backs the "Haul from" column on the plan buy list and the CLI.
- `resolve_build_station` and `attach_supply_columns` were **REMOVED** (along with the old global-station code). The `/plan` buy list runs `hauling.calculate_deficit` per product against that product's station for the at_station / elsewhere / to_buy split, then `attach_haul_breakdown` for the "Haul from" names; the `/materials` flat view is now a station-less aggregated BOM via `attach_owned_totals`.
...
- Shape: `{"targets": [...], "buy_set": [type_id, ...]}` — build/buy is global (`buy_set`) but the **build station is per target** (each target dict carries its own `build_station`); there is **no** top-level `build_station` key. `load()` is backward-compatible — a legacy bare-list file loads as `{"targets": <list>, "buy_set": []}`.
...
- `set_target_station(type_id, station_id)` — sets (or clears, with `None`) the `build_station` on **one** target. (The old global `set_build_station` is **REMOVED**.)
- A target dict carries `type_id`, `name`, `qty` (the driver), optional `runs` override, `me`, `structure_bonus`, and its own `build_station` (`int|None`). Build/buy is **global** (top-level `buy_set`); the build station is **per target**.
...
- Two tiers of commands:
  ...
  - **Build list**: `plan` — reads `build_list.json`, resolves every target, and prints **one block per product**: each target's tree is classified (ready/in-progress/blocked/buy) against **that target's own `build_station`** (`None` → owned-anywhere). Reads the global `buy_set` (`set(data["buy_set"])`), so a component toggled "buy" on the web `/materials` view shows up as a **buy line** here. Per block it prints "Building at: <name>" (or "— not set —") and a per-material "Haul from" breakdown on that product's buy list. There is **no** CLI command to set the station. Uses ESI inventory/jobs only if a token already exists (never forces the SSO flow); without it everything lands in blocked/buy.
  - The CLI has **no build/buy toggle UI** and no build-list materials command — build vs buy is set on the web `/materials` view, and the flat supply view (an aggregated owned-anywhere BOM: total required + to-buy) is web-only. `HELP` notes this. (The unrelated SDE-only `materials <name>` command is a single-blueprint material breakdown, not the build-list view.)
...
- Materials routes: `/materials` (`?view=tree` shows per-component **build/buy** toggles; `?view=flat` shows the flattened aggregated **bill of materials** — `flatten_material_tree(nodes, buy_set)` then `plan.attach_owned_totals` to net the gross requirement against everything owned **anywhere** (total required + to_buy + volume). **No station picker, no at-station/haul split** — the per-product station now lives only on the Plan page; the flat view is whole-list and station-less. Anonymous viewing is allowed (no auth → `owned_index` empty → to_buy == total)). `/materials/toggle/<id>` (POST) flips the global `buy_set` via `build_list.toggle_buy` and redirects back. Build/buy is a single global choice across the whole list and **drives the action plan** (a component set to buy moves from a build node to a buy line).
- Build-station route: `/build-station` (POST) persists the picker choice for **one product** via `build_list.set_target_station(type_id, station_id)` (empty value clears it → owned-anywhere) and redirects to `/plan`. Each product block on the Plan page renders the shared `templates/_station_picker.html` partial (carrying that block's `type_id`), which posts here on `<select>` change.
- `_get_station_list(p, character_id, corporation_id=None)` — builds the "Building at" picker options, returning `[{id, name}]` (up to **15**). Now **blueprint-based**: it gathers character blueprints (plus corp blueprints when `corporation_id` is given) via `esi.get_cached_blueprints`, ranks their locations with `esi.extract_blueprint_stations` (most blueprints first, so your main hub leads), resolves names, and caps at 15. **Falls back** to manufacturing-job history (`esi.fetch_industry_jobs` + `esi.extract_manufacturing_stations`) when no blueprints are found (e.g. a brand-new character); returns `[]` on total failure. The contract is unchanged, so the per-product picker and `/build-station` are untouched. The Plan page (`_compute_plan`) passes `corporation_id`, so corp blueprints are included there; the Materials flat view no longer uses a picker, and the legacy `shopping`/`profit` drill-down routes call it char-only (default `None`). This means those drill-down pages' station picker now lists character blueprint locations rather than job-history stations — a subtle UX change on those pages.
- `_resolve_targets()` / `_compute_plan()` — shared by `/plan` and `/api/plan`; mirror the CLI's qty-driven derivation and corp-or-personal asset source. `_compute_plan` returns a **list of blocks** (one per product) — `[{type_id, name, needed, station, buckets}]` — each classified against **that product's own** `build_station` (`None` → owned-anywhere) via `classify` + `enrich_buy` (location-aware) + `attach_haul_breakdown`, plus a shared `station_ctx` (`{"stations": [...]}`) feeding every per-product picker. Without ESI auth, `loc_index`/`jobs` are empty so everything lands in blocked/buy.
...
- Build Station selector on shopping lists (and a persisted **per-product** "Building at" picker on the Plan page) enables the location-aware hauling view (materials at station vs haul vs buy); the Materials flat view is a station-less owned-anywhere BOM
...
| `materials.html` | Materials view (`/materials`): tree with build/buy toggles + flat aggregated bill of materials (total required, to-buy owned-anywhere, volume); no station picker |
| `plan.html` | Action plan (`/plan`): one block per product, each with its own "Building at" picker, ready / in-progress / blocked / buy buckets, and "Haul from" column on the buy list |
| `_station_picker.html` | Shared per-product "Building at" station picker partial (carries the block's `type_id`; POSTs to `/build-station` on `<select>` change); included by `plan.html` |
...
112 tests across 8 test files, run with `python -m pytest tests/ -v`:
...
| `tests/test_plan.py` | Pure classifier: `merge_trees` (shared-component aggregation), `classify` (ready/in-progress/blocked/buy bucketing, missing-input detection, job/inventory netting, `buy_set` moves node to buy), `enrich_buy` (deficit split), `attach_owned_totals` (owned-anywhere total/to_buy for the flat BOM), `attach_haul_breakdown` (elsewhere→named haul list, sorting, no-mutation) | 23 |
| `tests/test_build_list.py` | Build-list persistence: load/save/`add_target` upsert-by-type_id/`remove_target`/`toggle_buy` (idempotent flip, preserves targets)/`set_target_station` (per-target build_station), legacy bare-list compat | 13 |
| `tests/test_app_plan.py` | Flask route wiring for the build list + action plan (per-product blocks) + materials view + per-product station picker (`/`, `/build-list/add`, `/build-list/remove`, `/plan`, `/api/plan`, `/materials` tree/flat, `/materials/toggle/<id>`, `/build-station`); `_get_station_list` blueprint ranking/naming, job-history fallback, 15-cap, char+corp count merge | 19 |
